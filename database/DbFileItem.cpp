...
bool DbFileItem::analizeFile(QWidget *parent)
    ...
    if(db.open())
        ...
        if(v.isValid())
            {
            int version = v.toInt();
            qDebug()<<"version: "<<version<< (version==this->version);
...
bool DbFileItem::executeScript(QSqlDatabase db, QString scriptFile)
    ...
    for(int i=0; i< commands.count();i++)
        ...
        if(!c.isEmpty())
            ...
            if(!q.exec())
                {
                qDebug()<<"error"<<db.lastError().text()<<q.lastQuery();
                return false;
                }
            }
        qDebug()<<i;
